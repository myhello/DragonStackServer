...
bool TcpEventServer::StartRun()
{
  ...
  if( m_Port != EXIT_CODE )
  {
    ...
    //初始化struct sockaddr_in结构体变量，然后把它作为参数传给函数evconnlistener_new_bind即可完成socket,bind,listen,accept等
    //LEV_OPT_REUSEABLE会把该socket设置成reuseable。这样，关闭该socket后，其他socket就能马上使用同一个端口
