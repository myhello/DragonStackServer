	...
	def vm_template_copy(self,vm_template,vm_name):
		...
		for vm in vms:
		    ...
		    if record["is_a_template"]:
		        ...
		        if record["name_label"].startswith(vm_template):
		            templates.append(vm)
		...
		print "  Selected template: ", self.session.xenapi.VM.get_name_label(template)
...
if __name__ == '__main__':
	...
	xen.vm_destroy("9ff47ea2-7522-d668-376b-9c00584be402")

	#print xen.find_uuid_by_lable('Win7')
	#xen.vm_update(xen.find_uuid_by_lable('Win7'),fields)
	#xen.vm_destroy(xen.find_uuid_by_lable('new'))
	#xen.vm_template_copy("Ubuntu","new")
	#print len(xen.get_template_list())
	#host,host_ref = xen.get_host_list()
	#print xen.host_details(host_ref)
	#vbds = xen.disks_by_vdb(xen.find_vbds(xen.find_uuid_by_lable('Win7')))
	#vbds = xen.session.xenapi.VDI.get_VBDs(xen.find_uuid_by_lable('Win7'))
	#print vbds
	#SR = xen.session.xenapi.SR.get_by_uuid('d0f860c-918f-51d1-7fb9-4f90622fb734')
	#vbds = xen.session.xenapi.VDI.get_VBDs(xen.find_uuid_by_lable('ubuntu')))
	#config = {'name_label':'my disk','name_description':'new disk','SR':SR,'type':'user','virtual_size':'104857600'}
	#print SR
	#print xen.session.xenapi.VDI.get_fields()
	#VDI = xen.session.xenapi.VDI.create(config)
	#SR = xen.session.xenapi.SR.get_all_records()
	#print SR.get_type()
	#for x in vdi_records:
	#		print vdi_records[x]['SR'].get_type()
	#vdi = xen.find_vdis(xen.find_vbds(xen.find_uuid_by_lable('Win7')))
	#xen.session.xenapi.VDI.resize(vdi,'214748400000')
